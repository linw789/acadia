...
impl Texture {
    ...
    pub fn load_textures(
        ...
        // Create staging buffers for each texture-info.
        let (staging_buffers, extents): (Vec<_>, Vec<_>) = texture_infos
            .iter()
            .map(|info| {
                let (pixels, extent) = match &info.src {
                    TextureSource::FilePath(path) => {
                        let texture_width: c_int = 0;
                        let texture_height: c_int = 0;
                        let original_color_component_count: c_int = 0;
                        let requested_color_component_count = match info.format {
                ...
                if let TextureSource::FilePath(_) = info.src {
                    unsafe {
        ...
        // Create texture holders for each texture info.
        let textures: Vec<_> = texture_infos
            .iter()
            .zip(extents.iter())
            .map(|(info, extent)| {
                ...
                let sampler_createinfo = vk::SamplerCreateInfo::default()
                    ...
                    .max_anisotropy(info.max_sampler_anisotropy)
